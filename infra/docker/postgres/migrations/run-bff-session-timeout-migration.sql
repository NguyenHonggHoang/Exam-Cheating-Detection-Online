-- Migration script to add session timeout columns to refresh_tokens table
-- This can be run manually on an existing database
-- Usage: docker exec -i exam-postgres psql -U postgres -d bff_db < infra/docker/postgres/migrations/run-bff-session-timeout-migration.sql
